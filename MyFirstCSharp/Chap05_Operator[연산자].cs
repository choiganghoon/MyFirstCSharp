...
namespace MyFirstCSharp
{
    public partial class Chap05_Operator : Form
    {
        ...
        {
            // 1. 대입 연산자: = 의 오른쪽에 있는 값(리터럴)을 왼쪽 데이터 타입 변수에 대입한다
            int iValue; // 첫 번째 데이터 담는 변수
            iValue = 10; // = 을 이용해서 10이라는 리터럴을 iValue 변수에 대입
        ...
        {
            // 2. +=: 리터럴을 왼쪽 변수에 있는 값과 합한 후 결과 를 다시 왼쪽 변수에 대입
        ...
        private void btnSS_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show(Convert.ToString(iValue++)); // 13 이아니라 12
            MessageBox.Show(Convert.ToString(++iValue)); // 14

            // 후위 증가 연산: 코드가 실행되고 난 다음에 자기자신에게 1증가 값이 대입
            // 전위 증가 연산: 코드가 실행될 때 자기자신에게 1증가 값이 대입

            // 전위 증가 연산(실시간 증가 연산 처리 가능) 사용하는 것을 권장
        }

        private void btnMM_Click(object sender, EventArgs e)
        {
            // 8. 전위 감소 연산자, 후위 감소 연산자
            int iValue = 10;
